...
struct Layer : public MaskMixin<T>
{
	...
	struct PathPoint {
		...
		Geometry::Point2D<int> m_preceding;
		Geometry::Point2D<int> m_anchor;
		Geometry::Point2D<int> m_leaving;
	...
	struct SubPath
	{
		SubPath(std::vector<PathPoint> points, bool closed)
			: m_points(points), m_closed(closed){};
		SubPath(bool closed) : m_closed(closed){};

		std::vector<PathPoint> m_points;
		bool m_closed; // privatize me
...
protected:
	...
	// TODO: Maybe we shoold convert from path resouce data into path point and back
	...
	std::unique_ptr<PathResourceData> generate_vector_mask()
	{
		...
		// Needs to be the first record for every vector mask
		...
		{
			bool closed = subPath.m_closed;
			auto subpathLengthRecord = std::make_shared<SubpathLengthRecord>(closed, subPath.getNumPoints());
			pathResourceData->m_records.push_back(subpathLengthRecord);
			...
			, std::vector<std::shared_ptr<IPathRecord>>::iterator& it)
	{
		...
		if (numPoints == 0)
		{
			// TODO: move all this validation stuff to the PathResourceData files when we read and/or construct!
			throw std::runtime_error("SubPathLength record found to have 0 bezier knot records/points! It must have at least one of them");
		}
		...
		{
			if (numPointsMet == numPoints) {
				break;
			}
			if (it == data->m_records.end()) {
